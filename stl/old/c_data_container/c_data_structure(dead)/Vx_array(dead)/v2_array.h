...
//二维数组类
template<typename T> class v2_array{//定义模板类型
private:
  pthread_rwlock_t rwlock_v2;//私有rw 锁
  bool stat_find;//查找状态
  int cur_pos;//空闲元素游标
  void** _pool_y;//数据池-不能直接访问--最高维的单维数组
...
public:
  ...
  v2_array(int max_volumex_x, int max_volume_y);//构造函数
  ~v2_array();//释构函数
