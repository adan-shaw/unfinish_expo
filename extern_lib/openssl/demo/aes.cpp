...
//aes ebc 模式(仍然是循环加密, 解密的ebc 模式!
//						每次'加密/解密'一个AES_BLOCK_SIZE 长度):
/*
...
void aes_ebc(void){
	...
	unsigned char *data=(unsigned char*)\
			malloc(AES_ENCRYPT_BUF_SIZE);//原数据buf
	unsigned char *encrypt=(unsigned char*)\
			malloc(AES_ENCRYPT_BUF_SIZE);//密文buf
	unsigned char *decrypt=(unsigned char*)\
			malloc(AES_ENCRYPT_BUF_SIZE);//解密文buf
	...
	//*********************************************************
	//初始化aes 加密钥匙key;
	//并明确aes 密钥的长度=AES_BLOCK_SIZE*8=AES_ENCRYPT_BUF_SIZE.
	AES_set_encrypt_key(key_str, AES_KEY_LEN, &key);


	//循环加密, 每次只能加密AES_BLOCK_SIZE长度的数据!!
	//所以: 定义,原数据buf or 密文buf or 解密文buf,都应该是AES_BLOCK_SIZE*n倍
	len = 0;
...
void aes_cbc(void){
	...
	strncpy((char*)sourceStringTemp,\
			"123456789 123456789 123456789 12a",sizeof(sourceStringTemp));
