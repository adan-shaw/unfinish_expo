...
/*
  ...
  注意: 系统设备总表初始化只会在系统启动的时候初始化一次, 而且会记录到临时表中, 
       这样下次查找设备的时候就不用if 判断设备是什么设备了, 
       当然设备标志就是表示设备是否正常的标志, 系统每次使用设备的时候都会判断设备是否繁忙, 
       再使用设备


  所以有下面的过程设计: 
  --No.1--进程表结构, 也叫PCB(进程控制块): sys\proc.h
  ...
  *2.fd_ofiles     -- fd 实体数组,根据fd 的int 数值直接按数组下标索引fd 实体位置, 极速索引
  关键解析1: 
    ...
    可能为向量, 但是我觉得会是数组: 第一数组不用消耗资源, 而且默认65536 多个void*
    实际的缓冲区大小为:
  ...
  *5.2.4个盘的原则是最好的, 因为其实分区表只有4条记录最多, 详情如下: 
  ...
  分区表记录例子: 80 01 01 00   |   0B FE BF FC   |   3F 00 00 00   |   7E 86 BB 00
